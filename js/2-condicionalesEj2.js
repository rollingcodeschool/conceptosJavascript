...
let mes = parseInt(
  prompt("Ingrese el número del mes (1 al 12): 1- enero, 2-marzo")
);
console.log(isNaN(mes));

// el contenido de mes es numero?
// !(false) = true
// !(true) = false
if (!isNaN(mes)) {
  if (mes > 0 && mes <= 12) {
    if (
      ...
      document.write(
        `El mes seleccionado (${mes}) tiene 28 o 29 días dependiendo del año`
      );
    } else {
      document.write(`El mes seleccionado (${mes}) tiene 30 días`);
    }
  } else {
    document.write(`Ingreso un número (${mes}) invalido`);
  }
} else {
